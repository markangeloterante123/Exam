<template lang="html">
  <div id="dashboard" v-if="loaded">
    ...
    <div class="actions">
      ...
      <nuxt-link to="/roles/create" class="success ml ten button pointer" v-if="checkPermission('add-role', true)">
        ...
        <div class="bottom_box">
          <div class="group_inline two nmb">
            ...
            <validation-provider tag="div" class="group select bordered nmb" name="type" :rules="{ required: true }" v-slot="{ errors }">
              ...
              <select class="input" name="type" autocomplete="off" v-model="record.type">
...
<script>
  ...
  export default {
      ...
      submit () {
        this.$refs.form.validate().then(success => {
          if (!success) {
            ...
            this.$axios.$post(`cms/roles/${this.$route.params.role_id}`, form_data).then(({ record }) => {
    ...
    mounted () {
      this.pagePermission(this, 'update-role', true)
    },
    asyncData ({ $axios, store, params }) {
      store.commit('global/settings/populateTitle', { title: 'Update Role' })

      return $axios.$get(`cms/roles/${params.role_id}`).then(({ record }) => {
        return {
          record: record,
