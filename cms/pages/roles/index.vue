<template lang="html">
  <div id="dashboard" v-if="loaded">
    ...
    <div class="actions">
      <nuxt-link to="/roles/create" class="success button pointer" v-if="checkPermission('add-role', true)">
    ...
    <table class="table">
      ...
      <tbody v-if="records.data.length > 0">
        <tr v-for="(data, key) in records.data" :key="key">
          <td>
            <nuxt-link :to="`/roles/${data.id}/update`" class="wrapper">
          ...
          <td class="buttons" width="208px">
            <div class="wrapper">
              <nuxt-link :to="`/roles/${data.id}/update`" class="item info pointer" v-if="checkPermission('update-role', true)">
              ...
              <div class="item ml cancel pointer" @click="toggleModalStatus({ type: 'delete_confirmation', status: true, item: { api: `cms/roles/${data.id}`, item_type: 'role' } })" v-if="checkPermission('delete-role', true)">
                <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" class="icon">
...
<script>
  export default {
    ...
    mounted () {
      this.pagePermission(this, 'roles')
    },
    asyncData ({ $axios, store, query }) {
      ...
      if (page) {
        api_route = `cms/roles?page=${page}`
      } else {
        api_route = 'cms/roles'
      }

      store.commit('global/settings/populateTitle', { title: 'Roles' })
